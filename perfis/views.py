...
def convidar(request, *args, **kwargs):
	try: 
		...
		raise exceptions.NotFound('Não foi encontrado um perfil com o id selecionado.')
	perfil_logado = get_perfil_logado(request)
	...
	raise exceptions.ParseError('Você não pode convidar o perfil com o id selecionado')
...
def aceitar(request, *args, **kwargs):
	perfil_logado = get_perfil_logado(request)
	try:
		convite = Convite.objects.fliter(convidado=perfil_logado).get(id=kwargs['convite_id'])
	except:
